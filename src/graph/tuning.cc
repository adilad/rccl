...
static const float baseLat  [NCCL_NUM_ALGORITHMS][NCCL_NUM_PROTOCOLS] = { { 39.4, 39.4, 46.0 }, { 37.0, 37.0, 39.1 } };
...
{ /* NVLINK */
  { /* Tree (LL/LL128/Simple)*/ {  2.5, 2.5, 5.5 }, /* Ring (LL/LL128/Simple)*/ {  .6, .6, 1.3 } },
  /* PCI */
  { /* Tree (LL/LL128/Simple)*/ { 3.2, 3.2, 7.1 }, /* Ring (LL/LL128/Simple)*/ { .7, .7, 2.5 } },
  /* NET */
  { /* Tree (LL/LL128/Simple)*/ { 10.8, 10.8, 30.0 }, /* Ring (LL/LL128/Simple)*/ {  16.6, 16.6, 46.7 } }
};
  ...
  for (int coll=0; coll<NCCL_NUM_FUNCTIONS; coll++) {
    ...
    for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) {
      ...
      for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
        ...
        float busBw = graphs[a]->nChannels * speed * 0.5;

        // Various model refinements
        if (a == NCCL_ALGO_RING && p == NCCL_PROTO_LL)    busBw *= 1.0/6.0;
        if (a == NCCL_ALGO_RING && p == NCCL_PROTO_LL128) busBw = std::min(busBw*120.0/128.0, ll128MaxBw[coll]);
        if (a == NCCL_ALGO_TREE) busBw = std::min(busBw*.5, comm->nNodes > 1 ? 70.0 : 90.0);
        if (a == NCCL_ALGO_TREE && p == NCCL_PROTO_LL) busBw *= 1.0/8.0;
